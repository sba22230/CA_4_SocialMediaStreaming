...
data = pd.read_csv("Data/TweetwithSentiment.csv")
...
if chart_visual == "Line Chart":
    ...
    if selected_status == "Smoked":
        fig.add_trace(
            go.Scatter(x=data.Country, y=data.Smokes, mode="lines", name="Smoked")
        )
    if selected_status == "Never_Smoked":
        fig.add_trace(
            go.Scatter(
                x=data.Country, y=data.Never_Smoked, mode="lines", name="Never_Smoked"
            )
        )
    if selected_status == "Unknown":
        fig.add_trace(
            go.Scatter(x=data.Country, y=data.Unknown, mode="lines", name="Unknown")
        )

elif chart_visual == "Bar Chart":
    if selected_status == "Formerly_Smoked":
        fig.add_trace(
            go.Bar(x=data.Country, y=data.formerly_smoked, name="Formerly_Smoked")
        )
    if selected_status == "Smoked":
